...
| `scripts/roster.py` | Multi-subcommand CLI for class rosters: `init` / `add` / `remove` / `rename` / `list` / `import` / `export` / `folders` (per-scholar inbox subfolders) / `sheet` (printable class list PDF) / `nametags` (printable name-tag grid PDF) / `groups` (random groupings) / `split` (delegates to `roster_split.py`) / `stamp` (per-scholar stamped copies of a worksheet). Storage in `%APPDATA%\PrintWatcher\rosters\<class>.csv` | `reportlab` for PDF outputs, `pypdf` for `stamp` |
